...
static bool isValidMemory(const void *inPtr) {
    // Read the memory
 ...
 */
static bool isReadableMemory(const void *inPtr)
{
    kern_return_t error = KERN_SUCCESS;
    ...
    error = vm_region_64(mach_task_self(),
                         &address,
                         &vmsize,
...
bool IsObjcObject(const void *inPtr)
{
    ...
    //
    bool isReadableMem = isReadableMemory(inPtr);
    bool isValidMem = isValidMemory(inPtr);
    if (isReadableMem && isValidMem)
    {
        return true;
    }
